...
public abstract class Kernel {
	...
	public static boolean ENABLE_IN_MEMORY_MODE = false;
	public static boolean ENABLE_LOG_REQUESTS = false;
	...
	private static void loadProperties(){
		...
		try {
			...
			checkProperties(PROPERTIES_LOG_PATH, Kernel.SCHEMAS_FOLDER + File.separator+DEFAULT_ROOT_SCHEMA_NAME + File.separator + LOG_FOLDER);
			...
			LOG_FILE = getConfig(PROPERTIES_LOG_PATH)+File.separator+LOG_FILE_NAME;
	...
	public static void loadRecovery(){
		createDirectory( getConfig(PROPERTIES_LOG_PATH));
		try {
	...
	public static void start(){
		...
		if(ENABLE_IN_MEMORY_MODE)loadAllDataInMemory();
	...
	private static HashMap<String, Class<? extends AbstractBufferPolicy>> initializeMapDefaultBufferPolicies(){
		...
		bufferPolicies.put(LRU2Q.class.getSimpleName(), LRU2Q.class);
	...
	public static void setBufferPolicy(Class<? extends AbstractBufferPolicy> bufferPolicy){
		...
		try {	
			...
			int coldPercet = 90;
			if(ENABLE_IN_MEMORY_MODE && Kernel.BUFFER_SIZE > 0) {
	...
	public static void loadAllDataInMemory() {
