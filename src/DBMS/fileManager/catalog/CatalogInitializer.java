...
public class CatalogInitializer {
	...
	//DEFINIÇÃO DA TABELA DE FRAGMENTOS
	public static String FRAGMENTS_NAME = "fragments";
	
	//DEFINIÇÃO DA TABELA DE DATABASE
	...
	private TableManipulate fragments_Table;
	private TableManipulate database_Table;
	...
	private void openSystemCatalogTables(){
		...
		if(systemTuples == null || systemTuples.isEmpty()){
			...
			//CRIANDO A TABELA DE FRAGMENTS
			FileTuple fragmentsModel = FileTuple.build(3, FRAGMENTS_NAME + TableManipulate.SEPARATOR+"0" + TableManipulate.SEPARATOR + "0");
			
			//CRIANDO A TABELA DE DATABASE
			FileTuple databaseModel = FileTuple.build(4, DATABASE_NAME + TableManipulate.SEPARATOR+"0" + TableManipulate.SEPARATOR + "0");
			...
			try {
				...
				//ESCREVENDO AS NOVAS TABELAS (FRAGMENTS E DATABASE)
				block.writeTuple(fragmentsModel);
				block.writeTuple(databaseModel);
		...
		for (FileTuple tuple : systemTuples) {
			...
			}else if(data[0].equals(FRAGMENTS_NAME)){
				
				fragments_Table = openTable(defaultSchema,2, FRAGMENTS_NAME, Integer.parseInt(data[1]), Integer.parseInt(data[2]), 
						new Column("fragments_id","int"),
						new Column("table_id_fk","int"),
						new Column("database_id_fk","int"),
						new Column("column","varchar"),
						new Column("operador","varchar"),
						new Column("valor","varchar"));

				fragments_Table.setSystemTable(true);
				
			}else if(data[0].equals(DATABASE_NAME)){
				
				database_Table = openTable(defaultSchema,2, DATABASE_NAME, Integer.parseInt(data[1]), Integer.parseInt(data[2]), 
						new Column("database_id","int"),
						new Column("name","varchar"),
						new Column("ip","varchar"),
						new Column("port","int"));
				

				database_Table.setSystemTable(true);
	...
	private void loadCatalog(){
		...
		//Kernel.getBufferManager().getBufferPolicy().flush(test.writeTuple(catalogTransaction, "1|AA|1.2|55"));
		//Kernel.getBufferManager().getBufferPolicy().flush(test.writeTuple(catalogTransaction, "2|AA|1.2|55"));
		//Kernel.getBufferManager().getBufferPolicy().flush(test.writeTuple(catalogTransaction, "3|AA|1.2|55"));
	//createSchema("escola");
	//createTable(Kernel.getCatalog().getSchemabyName("escola"), "baladas", new Column("id", "int"), new Column("nome", "varchar"), new Column("salario", "varchar"), new Column("dia", "int"));
