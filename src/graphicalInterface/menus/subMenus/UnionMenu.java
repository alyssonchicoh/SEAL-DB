...
public class UnionMenu extends AbstractSubMenu{
	...
	public UnionMenu(DBConnection connection) {
		super(connection);
		...
		if(drawPlanOperation!=null){
			...
			Kernel.getExecuteTransactions().execute(connection,new TransactionRunnable() {
				...
				public void run(ITransaction transaction) {
					...
					UnionOperation unionOp = (UnionOperation) drawPlanOperation.getPlanOperation().copy(new Plan(transaction), null);
					
					ITable tableResult = unionOp.execute();
