...
public class TestTPCHLoad {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Kernel.ENABLE_IN_MEMORY_MODE = false;
...
//					String tpchSourceFile = "E:/seal-db/seal-db/tpch/";
...
//						//Schemas 2
					//loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("nation"), tpchSourceFile);
					//loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("region"), tpchSourceFile);
					//loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("supplier"), tpchSourceFile);
//						//Schemas 2
				
						
					//Schemas 3
						/*
					loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("part"), tpchSourceFile);
					loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("customer"), tpchSourceFile);
					loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("partsupp"), tpchSourceFile);
					loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("order"), tpchSourceFile);
						*/
						
						
					//loadTable(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("lineitem"), tpchSourceFile);
					
					int total = 0;
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("nation"));
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("region"));
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("supplier"));	
				    total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("part"));
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("customer"));
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("partsupp"));
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("order"));
					total+=showDetails(transaction, Kernel.getCatalog().getSchemabyName("tpch").getTableByName("lineitem"));
				...
				public void onFail(ITransaction transaction, Exception e) {
					System.out.println("TestTPCHLoad -: ERROR->>>> " + count) ;
	...
	public static void loadTable(ITransaction transaction, ITable table,String fileName) throws FileNotFoundException, InterruptedException {
		fileName = fileName+table.getName()+".tbl";
