...
public class IntersectionMenu extends AbstractSubMenu{
	...
	public IntersectionMenu(DBConnection connection) {
		super(connection);
		...
		if(drawPlanOperation!=null){
			...
			Kernel.getExecuteTransactions().execute(connection,new TransactionRunnable() {
				...
				public void run(ITransaction transaction) {
					...
					IntersectionOperation op = (IntersectionOperation) drawPlanOperation.getPlanOperation().copy(new Plan(transaction), null);
					
					ITable tableResult = op.execute();
