...
public class ARC extends AbstractBufferPolicy {
	...
	void asserts() {
		if ((T1.size() + T2.size()) > capacity) {
			System.out.println("<<<<<< (T1+T2) superou a capacidade >>");
		}
		if (L1() + L2() > 2 * capacity) {
			System.out.println("<<<<<< (T1+T2+B1+B2) superou a capacidade >>");
		}
		if (T1.size() + B1.size() > capacity) {
			System.out.println("<<<<<< (T1+B1) superou a capacidade >>");
		}
		if (T2.size() + B2.size() > 2 * capacity) {
			System.out.println("<<<<<< (T2+B2) superou a capacidade*2 >>");
		}
	}
